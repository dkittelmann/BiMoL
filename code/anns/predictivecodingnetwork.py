...
# Save the model 
resultspath = '/home/kittelmad93/projects/results/pcn/model_checkpoint_{epoch:02d}_{accuracy:.2f}.keras'
...
def generate_dataset(img_dirt1, img_dirt2, class_namest1, class_namest2, label_dict, image_size = None, seed = None):       
    """Function to generate the dataset. Returns one dataset consisting of leading and trailing images."""
...
class AccuracyPerSequence(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # Get predictions and true labels for each batch
        for batch in self.model.train_data:
            inputs, targets = batch
            predictions = self.model.predict_on_batch(inputs)

            # Calculate accuracy per sequence for each frame and store them
            sequence_accuracies = []
            for j in range(predictions.shape[0]):  # iterate over each sequence in the batch
                accuracies = []
                for i in range(4):  
                    accuracy_metric = tf.keras.metrics.Accuracy()
                    accuracy_metric.update_state(targets[j, i], predictions[j, i])
                    accuracies.append(accuracy_metric.result().numpy())
                sequence_accuracies.append(accuracies)

            print(f"Epoch {epoch + 1} - Sequence-wise Accuracies: {sequence_accuracies}")
...
for i in range(3): #10 
    ...
    model.fit(train_dataset.batch(32).shuffle(99999).map(img_preproc).map(flatten),
            ...
            callbacks=[AccuracyPerSequence,checkpoint_callback]) # epochs=3
            #callbacks=[AccuracyPerSequence]) # ADD
